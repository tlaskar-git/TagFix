...
emit("entry-open", {
  ...
  quote: "the relay never moves\nwss://on.slobal.com/tunnel is permanent",
...
check("quote text shown", byId["quote-block"].textContent.indexOf("the relay never moves") === 0);
...
setTimeout(() => {
  ...
  byId["tag-text"].value = "wrong, on.slobal.com stays DNS only";
  keydown(byId["tag-text"], { key: "Enter", shiftKey: false });
  setTimeout(() => {
    const saved = lastCall("save_tag");
    check("save carries text, chips and target",
      saved && saved.text === "wrong, on.slobal.com stays DNS only" && saved.severity === "high" &&
