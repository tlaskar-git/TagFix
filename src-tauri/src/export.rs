...
mod tests {
    ...
    fn quote_tag_renders_as_a_blockquote_in_md() {
        let s = sweep_with(vec![quote_tag(
            1,
            "the relay never moves\nand is proxied through Cloudflare",
            "wrong, it stays DNS only",
        )]);
        let md = render_fixlist_md(&s, "d");
        assert!(md.contains("> the relay never moves\n> and is proxied through Cloudflare\n"));
    ...
    fn quote_tag_renders_as_a_styled_blockquote_in_html() {
        let s = sweep_with(vec![quote_tag(1, "the relay never moves", "wrong")]);
        let html = render_fixlist_html(&s, "d", |_| Some(vec![1]));
        assert!(html.contains("<blockquote class=\"quote\">the relay never moves</blockquote>"));
    ...
    fn mixed_sweep() -> Sweep {
        let mut q = quote_tag(
            1,
            "The relay never moves; wss tunnel is permanent\nand proxied through Cloudflare.",
            "Wrong: on.slobal.com must stay DNS only.",
        );
    ...
    fn feedback_md_matches_the_agreed_shape() {
        ...
        assert!(md.contains("> The relay never moves; wss tunnel is permanent\n"));
        assert!(md.contains("Wrong: on.slobal.com must stay DNS only."));
        assert!(md.contains(
    ...
    fn feedback_txt_drops_the_markdown_syntax() {
        ...
        assert!(txt.contains("    The relay never moves; wss tunnel is permanent\n"));
        assert!(!txt.contains("> "));
        assert!(!txt.contains("["));
        assert!(txt.contains(
